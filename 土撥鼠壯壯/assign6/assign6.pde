...
void initGame(){
	...
	for(int i = 0; i < enemies.length; i++){
		...
		switch(floor(i/2)){
			...
			case 1: enemies[i] = new Dinosaur(newX, newY); break;
			case 2: enemies[i] = new Robot(newX, newY); break;
		}
	}
	...
	for(int i = 0; i < items.length; i++){
		...
		float newY = SOIL_SIZE * ( i * 4 + floor(random(4)));
		items[i] = (random(1) < 0.5f) ? new Cabbage(newX, newY) : new Clock(newX, newY);
	}
}
